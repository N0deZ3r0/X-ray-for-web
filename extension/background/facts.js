...
import { t, n as скл, подробность } from './i18n.js';
...
const ПРАВИЛА = [
  ...
  {
    id: 'cookie-id',
    // «Долгоживущий» — это утверждение о сроке, и говорить его можно только
    // когда срок известен и он действительно долгий.
    //
    // Первая редакция требовала лишь «это не удаление». На fingerprint.com под
    // неё попала проба возможностей `cookietest`, поставленная вообще БЕЗ срока
    // жизни: сессионная кука, которая исчезнет с закрытием вкладки, была
    // названа долгоживущим идентификатором. Кука без срока — не улика.
    когда: (e) => e.surface === 'cookie.write' && долгоживущая(e.arg),
    текст: t('fact_cookie_id'),
    значение: (e) => e.arg,
  },
  {
    id: 'storage-id',
    // Только localStorage: sessionStorage исчезает с закрытием вкладки, и
    // говорить про него «записал данные в хранилище браузера» в разделе про
    // слежку значило бы обвинить в постоянстве непостоянное (предел 3в).
    когда: (e) => e.surface === 'localStorage.setItem',
    текст: t('fact_storage'),
    значение: (e) => e.arg,
    // Из пятнадцати записей в улику попадала первая по времени. На
    // fingerprint.com ею оказался `cookieChoice` в 15 символов — то есть
    // СОГЛАСИЕ НА КУКИ, самое безобидное, что сайт мог записать, — при том что
    // рядом в том же журнале лежал `_vid_lr` в 4037 символов.
    //
    // Это тот же промах, что был с кукой: представителя выбирал порядок
    // событий, а не их содержание. Там он лечился сужением условия, здесь
    // сужать нечего — «записал данные» верно про любую запись. Значит выбирать
    // надо представителя, и по измеримому признаку, а не по догадке о том, что
    // «похоже на идентификатор».
    //
    // Признак — размер. Он измерен прибором, а не оценён; и утверждение при
    // этом не растёт: факт по-прежнему говорит «записал данные», просто
    // показывает самую крупную запись вместо самой ранней.
...
// Разбирается КОД, который пишет инструмент, а не слова, которыми это потом
// показывают. Первая редакция искала русское «, до <дата>» — то есть готовый
// текст показа. Когда инструмент перешёл на коды, она перестала совпадать
// молча, и на fingerprint.com проба возможностей `cookietest`, поставленная и
// тут же стёртая датой 1970 года, попала в свод как «долгоживущий
// идентификатор». Третий случай одного и того же: логика на тексте показа.
function удалениеКуки(arg) {
  const s = String(arg || '');

  // «имя | until:Thu, 01-Jan-1970 00:00:01» — дата в прошлом значит стирание.
  const до = / \| until:(.+)$/.exec(s);
  if (до) {
    const когда = Date.parse(до[1]);
    return Number.isFinite(когда) && когда < Date.now();
  }

  // «имя | days:0» — max-age=0 это тоже стирание.
  const дней = / \| days:(\d+)$/.exec(s);
  if (дней) return Number(дней[1]) === 0;

  return false;
}

// Порог, ниже которого «долгоживущий» говорить нельзя. Месяц выбран как
// граница между технической пробой (минуты, часы, сутки) и меткой, которая
// переживёт возвращение на сайт.
const ДОЛГО_ДНЕЙ = 30;

// Известен ли срок жизни и достаточно ли он долог, чтобы называть куку
// идентификатором. Кука БЕЗ срока — сессионная: она исчезнет с закрытием
// вкладки, и утверждать про неё «долгоживущий» нечем.
// Размер записи в хранилище. Инструмент кладёт его в аргумент КОДОМ
// ' | chars:N' — сравниваем число, а не отрисованные слова: слова зависят от
// языка браузера, и сравнение по ним ломается на английском (предел 7б).
// Неизвестный размер даёт -1, поэтому запись с известным размером всегда
// выигрывает у записи без него.
function размерЗаписи(arg) {
  const m = /\| chars:(\d+)$/.exec(String(arg == null ? '' : arg));
  return m ? Number(m[1]) : -1;
}

// Самая крупная запись из числа улик. Если размер неизвестен ни у одной,
// вернётся первая — то есть прежнее поведение, а не пустота.
function крупнейшая(улики) {
  let лучшая = улики[0] || null;
  let максимум = -1;
  for (const e of улики) {
    const n = размерЗаписи(e.arg);
    if (n > максимум) {
      максимум = n;
      лучшая = e;
    }
...
function долгоживущая(arg) {
  const s = String(arg || '');

  const дней = / \| days:(\d+)$/.exec(s);
  if (дней) return Number(дней[1]) >= ДОЛГО_ДНЕЙ;

  const до = / \| until:(.+)$/.exec(s);
  if (до) {
    const когда = Date.parse(до[1]);
    if (!Number.isFinite(когда)) return false;
    return когда - Date.now() >= ДОЛГО_ДНЕЙ * 86400 * 1000;
  }

  // Срок не указан вовсе — сессионная кука. Не улика.
  return false;
}
...
export function выводитьФакты(session) {
  ...
  for (const правило of ПРАВИЛА) {
    ...
    // Представителя выбирает само правило, если ему есть по чему выбирать.
    // Порядок событий — не признак важности: он выдвигал в улику самую раннюю
    // запись, а ранняя обычно самая скучная.
    const источник =
      (правило.выбор && правило.выбор(улики)) || улики.find((e) => e.result) || улики[0];
    let значение = null;
    try {
    ...
    факты.push({
      id: правило.id,
      text: правило.текст,
      // Инструмент пишет подробности кодами; в факте человек должен увидеть
      // слова. До этой правки в своде стояло «n:4» и «cookieChoice | chars:15».
      value: значение == null || значение === '' ? null : подробность(String(значение)),
  ...
  //
  // Перекрытые на экземпляре в знаменатель НЕ идут. Обёртка на прототипе у них
  // стоит, но чтение до неё не доходит: кто-то положил собственное свойство
  // поверх. Считать их наблюдаемыми значило бы занижать долю снятого и обещать
  // зрение, которого нет (предел 3б).
  const наблюдаемые = (session.installedSurfaces || []).filter(
    (x) => ГРУППЫ_ОТПЕЧАТКА.has(x.group) && !x.shadowed
  );

  const перекрытыхОтпечатка = (session.installedSurfaces || []).filter(
    (x) => ГРУППЫ_ОТПЕЧАТКА.has(x.group) && x.shadowed
  ).length;
  ...
  return {
    ...
    coverage: {
      ...
      // Сколько поверхностей выпало из знаменателя из-за чужой подмены.
      // Ноль — обычное состояние; всё, что больше, человек должен видеть.
      перекрыто: перекрытыхОтпечатка,
      список: [...снятые].sort(),
    },
  };
}
