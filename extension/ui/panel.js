...
(() => {
  ...
  function renderFilters(surfaces) {
    ...
    //
    // Число у чипа — это всегда «сколько будет видно после нажатия». У всех
    // остальных так и было, а здесь стояло число ОТБРАСЫВАЕМЫХ записей: на
    ...
    if (чужих) {
      ...
      box.appendChild(sep);
      box.appendChild(
        чип(t('filter_no_ext'), surfaces.length - чужих, фильтрТолькоСайт, () => {
