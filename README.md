...
![checks](https://img.shields.io/badge/checks-249-2f9e44)
...
> **The interface follows your browser's language.** Chrome in Russian gives a Russian
> panel and a Russian report; anything else gives English. Both come from the same
> source — `tools/messages.mjs`, where the two languages sit side by side — and CI fails
> if the generated `_locales` drift from it. The foundation documents and the code
...
3. **Record this site**. Chrome asks for permission for this site
...
6. **Save report** gives a readable report, **Save JSON** the full data.
...
**249 checks** in eight suites, with no dependencies at all — plain `node`. The most
...
extension/     the extension itself (this is the folder Chrome loads)
  ...
  _locales/    ru and en, generated by tools/messages.mjs
