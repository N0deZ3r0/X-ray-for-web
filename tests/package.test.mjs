...
console.log('получатель вызова');
{
  ...
  // получателя вызова. Пока имя поверхности было прибито к мете, всё
  // подписывалось «localStorage», и запись в sessionStorage — исчезающая с
  // закрытием вкладки — выглядела постоянной.
  //
  // Проверка по исходнику: поднять здесь весь мир страницы дороже, чем стоит,
  // а инвариант короткий и легко нарушается обратно одной правкой меты.
  const src = читать('instrument', 'main.js');
  const блок = src.slice(src.indexOf("const STORAGE = proto('Storage')"), src.indexOf("proto('IDBFactory')"));
  проверка(
    'хранилища различаются по получателю, а не по имени в мете',
    /поверхностьПо/.test(блок) && /sessionStorage/.test(блок),
    блок ? 'поверхностьПо не задана' : 'блок хранилищ не найден'
  );
  проверка(
    'обе поверхности хранилищ объявлены в реестре',
    /sessionStorage.setItem/.test(src) && /sessionStorage.getItem/.test(src),
    'sessionStorage не объявлен'
  );
  проверка(
    'получатель доезжает до записи',
    /function record\(meta, args, out, startedAt, приёмник\)/.test(src),
    'record не принимает получателя'
  );
}
console.log('');
